# Local alphaXiv Copy: NECO: Neural Collapse Based Out-of-Distribution Detection
...
, Gianni Franchi
⋄ ∗
U2IS Lab ENSTA Paris
⋄
, Palaiseau, FRANCE
SafranTech
†
, Chateaufort 78117, FRANCE
{first.last}@enstaparis.com
⋄
, safrangroup.com
†
ABSTRACT
...
• We represent the training and testing sets as D
l 
= (x
i
, y
...
i 
∈ J0, CK denotes its associated class identifier,
...
distributions, denoted as P
l 
and P
τ 
.
• In the context of anomaly detection, assumeion that P
l 
and P
τ 
exhibit a high degree
of similarity. However, we also introduce another test dataset denoted as D
OOD 
=
(x
OOD
...
joint distribution, referred to as P
OOD
, which is distinct from both P
l 
and P
τ 
.
...
(x
i
) denotes the output of the DNN when applied to the input image x
i
.
• To simplify the discussion, we assume that the DNN can be divided into two parts: a feature
extraction component denoted as h
ω 
(·) and a final layer, which acts as a classifier and is
3

Published as a conference paper at ICLR 2024
denoted as g
ω 
(·). Consequently, for any input image x
i
, we can express the DNN’s output
as f
ω 
(x
i
...
• In the context of image classification, we consider the output of h
ω 
...
for image x
i 
with D the dimension of the feature space.
• We define the matrix H ∈ M
n
l
,D 
(R) as containing all the h
ω 
(x
i
) values where x
...
c
l 
as a dataset consisting of data points belonging to class c, and H
c 
repre-
...
D 
and the empirical class means μ
c 
=
1/card(D
c
...
empirical global mean μ
OOD
G 
and the OOD empirical class means μ
OOD
c 
, and the OOD
feature matrix H
...
,D 
(R).
In the context of unsupervised OOD detection with a post-hoc method, we train the function f
ω 
(·)
using the dataset D
l
. Following the training process, we evaluate the performance of f
ω 
on a com-
bined dataset consisting of both D
OOD 
and D
τ 
. Our objective is to obtain a confidence score that
enables us to determine whether a new test data point originates from D
OOD 
or D
τ 
.
3.2 NEURAL COLLAPSE
Throughout the training process of f
ω 
, it has been demonstrated that the latent space, represented by
the output of h
ω 
, exhibits four distinct properties related to NC. In this subsection we delve deeper
...
respective class means, effectively making Σ
W 
approach zero. To evaluate this property during
...
Here, [.]
† 
signifies the Moore-Penrose pseudoinverse. While the authors of Papyan V (2020) state
that the convergence of Σ
W 
towards zero is the key criterion for satisfying NC1, they also point out
that Tr
h
Σ
W 
...
adopted since it scales the intra-class covariance matrix Σ
W 
(representing noise) by the pseudoin-
verse of the inter-class covariance matrix Σ
B 
(representing signal). This scaling ensures that NC1
...
Here, ∥ · ∥
2 
represents the L2 norm of a vector, | · | denotes the absolute value, ⟨·, ·⟩ is the inner
product, and δ
·· 
is the Kronecker delta symbol. The convergence to the Simplex ETF is assessed
4
...
(x) = W × h
ω 
(x), where W is the matrix of the last fully con-
nected layer. Cook et al. (2020); Wang et al. (2022) have highlighted that features h
ω 
(x) can
5
...
be decomposed into two components: h
ω 
(x) = h
ω 
(x)
...
P ∈ M
d,D 
(R) spanned by the d eigenvectors corresponding to the largest d eigenvalues of the co-
...
Theorem 4.1 (NC1+NC2+NC5 imply NECO). We consider two datasets living in R
D 
, {D
OOD
, D
...
◦h
ω 
)(·) that satisfy NC1, NC2 and NC5. There ∃ d ≪ D for PCA on D
τ 
s.t.
NECO(μ
OOD
G 
) = 0. Conversely, for x ∈ D
τ 
and considering x̸ =
⃗
 0 we have that NECO(x)̸ = 0.
6
...
Remark 4.1. We have established that NECO(μ
OOD
G 
) = 0, which does not imply that NECO(x) =
0 for all x ∈ D
OOD
. However, in addition to NC5, we can put forth the hypothesis that NC2 also
...
Published as a conference paper at ICLR 2024
Baseline Methods. In our evaluation, we compared our method against twelve prominent post-
...
Energy 93.03 31.65 92.13 34.15
 99.04 4.76 92.65 36.90 90.38 43.72 93.45 30.24
Energy+ReAct 93.08 31.25 92.08 34.50 99.02 4.91 92.56 36.94 90.19 44.25 93.39 30.37
ViM 94.01 28.95
 91.63 38.22 99.59 2.00 92.73 34.00 89.67 44.12 93.56 29.46
...
Mahalanobis 94.00 30.90
 91.69 37.93 99.67 1.55 91.38 38.48 88.66 46.09 93.08 30.99
...
ASH-P 93.31 29.00 
91.55 37.58 98.75 6.25 92.96 33.50 90.07 41.81 93.33 29.63
...
NECO (ours) 94.53 25.20 92.86 32.44 99.34 3.26 93.15 33.98 
90.38 42.66 94.05 27.51
SwinV2
Softmax score 61.09 89.60 81.72 60.91 88.59 47.66 81.24 66.85 81.09 68.27 78.75 66.66
...
Energy 98.63 5.93 99.92 0.21
 99.28 3.07 92.68 26.37 96.68 15.72 94.68 21.05
...
GradNorm 95.82 10.92 99.85 0.48 97.84 5.7 
92.10 26.20 94.85 16.69 93.48 21.45
Mahalanobis 98.73 5.95 95.52 18.61 97.13 12.28 95.42 24.02 93.91 37.11 94.67 30.57
KL-Matching 83.48 20.65 95.07 6.09 89.28 13.37 
79.47 38.34 80.19 41.73 79.83 40.04
ASH-B 95.15 17.49 99.09 4.81 97.12 11.15 
78.07 53.01 82.99 53.97 80.53 53.49
ASH-P 97.89 7.22 99.90 0.26 98.90 3.74 85.12 29.82 97.04 14.06 91.08 21.94
ASH-S 97.56 7.73 99.89 0.32 98.73 4.03 
83.30 30.96 97.04 14.55 90.17 22.56
...
Energy 85.46 58.68
 93.96 29.71 89.71 44.20 75.25 83.76 76.40 90.77 75.83 87.27
...
GradNorm 60.83 69.98 78.28 42.83 69.56 56.41 
72.62 84.64 69.66 91.58 59.55 88.11
...
Julian Bitterwolf, Maximilian Müller, and Matthias Hein. In or out? fixing imagenet out-of-
...
quences with ransac-based outlier rejection scheme. In Intelligent Vehicles Symposium, pp.
...
work for detecting out-of-distribution samples and adversarial attacks. In S. Bengio,
...
A Theoretical justification 15
B Details on Baselines Implementations 15
C Additional Experiment Details 17
C.1 OOD datasets . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 17
C.2 NECO pseudo-code . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 17
C.3 Effect of MaxLogit multiplication . . . . . . . . . . . . . . . . . . . . . . . . . . 18
C.4 Fine-tuning details . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 18
C.5 Dimension of Equiangular Tight Frame (ETF) . . . . . . . . . . . . . . . . . . . . 19
C.6 Classification Accuracy . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 21
D Details about Neural Collapse 21
D.1 Additional neural collapse properties . . . . . . . . . . . . . . . . . . . . . . . . . 21
D.2 Neural collapse additional results . . . . . . . . . . . . . . . . . . . . . . . . . . . 22
E Complementary Results 24
14
...
P CA
1 
as the projection of z
1 
onto a subspace obtained by
applying PCA to an external dataset D of same dimensionality as z
1
. This subspace has dimensions
limited to d ≤ D.
Lemma A.1 (Orthogonality conservation). We consider a single dataset, D
1 
= {h
i
}
...
D 
,
along with two vectors, {z
1
, z
2
} ∈ R
D 
, that are not necessarily part of D
1 
...
= 0. Let us define P ∈ M
D,d
(R) the projection matrix of
the P CA.
By definition P is an orthogonal matrix, hence P 
⊤
P = I
d 
and P P 
...
and I
d 
are the identity matrices of M
D×D 
(R) and M
d×d
(R), respectively. We
...
Theorem A.2 (NC1+NC2+NC5 imply NECO). We consider two datasets living in R
D 
,
{D
OOD
...
◦ h
ω 
)(·) that satisfy NC1, NC2 and NC5. There ∃ d ≪ D for
PCA on D
τ 
s.t. NECO(μ
OOD
...
⃗
 0 we have that
NECO(x)̸ = 0.
Proof. Assuming, for the sake of contradiction, that we have ∀d ≤ D s.t. NECO(μ
OOD
G 
)̸ = 0 and
NECO(x) = 0 for data points x in D
τ 
. Given a PCA dimension d s.t. d ≥ C and based on the
mathematical underpinnings of PCA (i.e., linear nature of projections) coupled with the NC1+NC2
assumptions, most data points x in D
τ 
satisfy the condition that ∥P h
ω 
(x)∥ is maximized and there-
...
dimensional space that is aligned with μ
c
. As a result, the set of vectors {μ
c
}
C
c=1 
...
d = C, should be spanned by the set {μ
PCA
c 
}
C
c=1
. Here, for each class c, μ
PCA
c 
represents the PCA
projection of μ
...
principal space P and a null space P 
⊥
. The ViM score is then computed by using the projection
...
AUROC↑ FPR↓
ViT 
NECO w/o Maxlogit 93.27 30.05 89.02 46.96 99.48 1.47 89.90 38.24 85.89 52.15 91.51 33.77
NECO 94.53 25.20 92.86 32.44 99.34 3.26 93.15 33.98 90.38 42.66 94.05 27.51
SwinV2
NECO w/o Maxlogit 63.17 87.95 75.52 69.63 90.72 36.37 81.59 57.19 80.52 60.77 78.31 62.38
NECO 65.03 80.55 82.27 54.67 91.89 34.41 82.13 62.26 81.46 64.04 80.56 59.19
DeiT 
NECO w/o Maxlogit 56.21 95.80 60.02 96.68 71.41 84.49 57.29 84.10 55.54 82.74 60.10 88.76
...
up steps, 256 batch size, 0.9 momentum, and an initial learning rate of 2x10
−2
. For CIFAR-10
...
model is trained for 200 epochs with 128 batch size, 5x10
−4 
weight decay and 0.9 momentum. The
...
(top1%)
ImageNet 
ViT-B/16 Transformer ImageNet-21k 83.84
SwinV2-B/16 Transformer ImageNet-21k 81.20
DeiT-B/16 Transformer ImageNet-21k 81.80
CIFAR-10 
ViT-B/16 Transformer ImageNet-21k 98.89
ResNet-18 Residual Convnet - 95.46
CIFAR-100 
ViT-B/16 Transformer ImageNet-21k 92.16
...
2
2 
signifies the square of the L2 norm.
...
details). Despite a very low initial value (2 × 10
−1 
for the worst example), we observe that NC5
...
OpenOOD Benchmark — ImageNet-1K Challenge. OpenOOD
1 
is a recent test bench focusing
...
Published as a conference paper at ICLR 2024
Covariate Shift Near OOD Far OOD Global
Model Method 
ImageNet-V2
AUROC↑ FPR↓
ImageNet-C
...
NECO (ours) 
58.69 89.00 73.48 69.57 86.45 47.96 72.87 68.84 93.38 30.98 87.52 41.54 90.45 36.26 92.86 32.44 99.34 3.26 97.55 12.99 96.58 16.23 86.16 40.97
SwinV2
Softmax score 
57.76 90.14 75.99 67.42 78.35 64.50 70.70 74.02 78.54 71.91 70.54 82.39 74.54 77.15 81.72 60.91 88.59 47.66 85.08 57.70 85.13 55.42 77.07 67.83
...
Mahalanobis 
57.14 90.83 70.04 83.55 84.58 55.69 70.59 76.69 78.76 79.11 68.94 88.37 73.85 83.74 84.51 63.35 89.81 57.10 88.20 58.95 87.51 59.80 77.75 72.12
KL-Matching 
54.74 90.95 64.08 81.73 73.40 70.87 64.07 81.18 72.29 75.40 62.96 83.82 67.62 79.61 75.30 71.69 82.93 58.29 80.26 65.74 79.50 65.24 70.75 74.81
...
ASH-P 
45.39 95.91 36.69 96.14 25.86 98.51 35.98 96.85 29.17 98.37 37.20 97.51 33.19 97.94 26.51 98.90 20.73 99.28 22.35 99.22 23.20 99.13 30.49 97.98
...
NECO (ours) 
58.04 89.86 77.16 63.05 83.67 56.36 72.96 69.76 77.95 66.69 70.63 79.47 74.29 73.08 82.27 54.67 91.89 34.41 86.98 47.96 87.05 45.68 78.57 61.56
DeiT
...
GradNorm 
46.54 94.62 56.39 80.16 26.00 98.43 42.98 91.07 34.63 97.48 43.45 95.91 39.04 96.69 60.22 85.50 46.24 95.00 27.40 97.89 44.62 92.80 42.61 93.12
